/*  DUGL Dos Ultimate Game Library - blur test Sample */
/*  sample testing the blur16 function */
/*  First working version 31 march 2007 */
/*  Thanks to Laaca and NTOSKRNL.VXE for ideas and testing :) */
...
//	if (!LoadGIF(&LightImg,"nightf.gif",&palette))
//	  { printf("jeux1.gif error\n"); exit(-1); }
        // build col conversion tables
        PrBuildTbColConv(&palette,0.6);
        
        // create needed surfaces for blur-------------
        // --------------------------------------------
        if (!CreateSurf(&BlurImg8, LightImg.ResH, LightImg.ResV,8)) {
           CloseVesa(); UninstallTimer(); UninstallKeyboard();
           printf("no mem\n"); exit(-1);
        }
        if (!CreateSurf(&BlurImg16, LightImg.ResH, LightImg.ResV,16)) {
           CloseVesa(); UninstallTimer(); UninstallKeyboard();
           printf("no mem\n"); exit(-1);
        }
        if (!CreateSurf(&ToBlurImg16, LightImg.ResH, LightImg.ResV,16)) {
           CloseVesa(); UninstallTimer(); UninstallKeyboard();
           printf("no mem\n"); exit(-1);
        }
...
//        SetSurf(&VSurf[0]);
	...
	PrBuildTbDegCol(&palette,0.6);
        ...
        // init the blur
        ConvSurf8ToSurf16Pal(&BlurImg16, &LightImg,&palette);
        SrcBlur16=&BlurImg16;
        DstBlur16=&ToBlurImg16;
        // convert original image to 8bpp
        ConvSurf16ToSurf8(&BlurImg8, &BlurImg16);

        // start the main loop
        for (j=0;;j++) {
          // wait 180 frames then start bluring each 60 frames
          if (j%60==0 && j>180) {
            BlurSurf16(DstBlur16, SrcBlur16);
            ConvSurf16ToSurf8(&BlurImg8, DstBlur16);
            TmpSurf= SrcBlur16;
            SrcBlur16= DstBlur16;
            DstBlur16= TmpSurf;
          }
          ...
          // wait retrace if fps is greater than the screen refresh
         if (deltaTime==0 || (TimerFreq/deltaTime)>=CurModeVtFreq/*(1.0/avgFps)>=CurModeVtFreq*/)
           {
	     ...
	     SetSurf(&VSurf[(j+1)%3]);
           ...
           if (BoutApp(0xf) && (KbFLAG&SHIFT_PR) && (KbFLAG&CTRL_PR))
              SavePCX(&VSurf[j%3],"blur.pcx",&palette);

           // set the view of the sprites for the current drawing surf
           SetSurfRView(&CurSurf, &GridView);
           
           // build points, polygones
           BuildPtsPolys();

           // draw the blured Surf
           for (i=0;i<NbPolys;i++)
              Poly(&ListPolys[i], &BlurImg8/*&LightImg*/, POLY_TEXT/*POLY_DEG_TEXT*/, rouge);
           // draw the grid
           ...
           FREE_MMX();
           sprintf(text,"grid size %02ix%02i, fps %03i, screen refresh %03i",GridNbCols,
        ...
        DestroySurf(&BlurImg8);
        DestroySurf(&ToBlurImg16);
        DestroySurf(&BlurImg16);
        DestroySurf(&LightImg);
        CloseVesa();
